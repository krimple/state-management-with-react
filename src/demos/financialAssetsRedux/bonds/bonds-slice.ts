...
export const updateBond = createAsyncThunk('bonds/updateBond', async (bond: BondAsset, thunkAPI) => {
    ...
    if (!updateResponse.ok) {
        thunkAPI.abort(`Bond ${bond.id} cannot be updated. ${updateResponse.status} - ${updateResponse.statusText}`);
    }
