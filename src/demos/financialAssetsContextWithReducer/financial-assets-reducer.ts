...
export async function doSaveAsset(
  asset: FinancialAssetType,
  ...
  if (!response?.ok) {
    throw new Error(
      `doSaveAsset failed. ${response?.status} - ${response?.statusText}`,
    );
  }
...
export default function financialAssetsReducer(
